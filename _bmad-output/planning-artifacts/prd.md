...
- FR42: An agent that reads another party's pod writes a read receipt (who read, what, when) into the **data subject's** pod, at `access-log/`, holding `acl:Append` and never `acl:Write` — making the decision to revoke an informed one rather than a theoretical right. This is a voluntary accountability convention, not enforcement: the Pod server does not surface per-resource read logs to owners, so a non-cooperating reader is not detected by this mechanism. _(Corrected 2026-08-11: this line previously said the receipt lands in the reader's **own** pod. That contradicted architecture.md BP-1 as amended 2026-08-02 and the shipped implementation in `mcp-connector/src/receipt.js`, and it inverted the principle — a receipt held by the audited party can be quietly deleted by them. Append-without-Write in the subject's pod is the deliberate exception to BP-6's single-writer invariant, and is the reason the exception exists. Journal integrity is Story 8.9.)_
