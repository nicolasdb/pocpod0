...
- **Anagnorisis is a validation outcome, confirmed against `references/schema-v1.md`** (the finalized Tinder→Rate contract, supplied 2026-08-26): `validation` is 4-way exclusive (`pending`/`validated`/`rejected`/`anagnorisis`). Tapping 🏛️ in the drag-up tray commits it directly, same write path as a swipe. Went through two intermediate reverts in-session (outcome → flag → outcome) before this doc settled it — see the story file's Dev Notes for the full back-and-forth.
- **`revisit`/`priority` are separate, non-exclusive flags** — a multi-select tray toggle, written to a new `triage_flags` field only when non-empty, ridden along with whichever outcome comes next. Deliberately not the pipeline's own `tags` field (different meaning, already populated on every gist with topical keywords). The restored comment field writes to `note`, also new, also schema-v1.md-documented.
- **Undo re-patches** the file back to prior `validation`/`note`/`triage_flags` rather than only rolling back local state, and its enable-gate moved off "unsaved batch" (which no longer exists) onto "there is a last swipe."
- **Live bugs found+fixed during dogfooding:** demo mode's synthetic path was leaking into real logins via localStorage; the real pipeline's single-line `raw = """text"""` blocks broke the parser's multi-line detection, silently corrupting every gist after the first; a failed-write retry duplicated the gist in the deck instead of resetting the pointer. Also added: a back button on the deck screen (previously no way out once "nothing pending" or mid-deck).
- **No schema gap:** `[gist_session].ingested` absent from the live file was flagged mid-session as a possible mismatch — schema-v1.md confirms absent = `false` is the documented default, not a gap.
