...
- **The real open question is narrower and harder: can it be Append-only at all?** `podClient.appendFile` is a **read-then-overwrite**, not a protocol append — it calls `getFile` then `overwriteFile`, so the receipt path structurally needs Read **+** Write. Granting `acl:Append` alone would not tighten the journal; it would stop receipts working. **Grant and write path must move together or not at all.**
- **Three candidate paths, one must be tested live:** POST-per-receipt into the container (`acl:Append` on a container permits creating new resources; journal becomes many resources instead of one file); RDF receipts + insert-only N3 `PATCH` (requires abandoning the JSONL shape that mirrors Epic 5's `receipt.py`); or keep RW and describe it honestly. **Path C is a legitimate outcome, not a failure.**
- **The backoffice gap was never the real blocker.** 8.6 attributed Append-only's absence to the Epic 7 UI offering no such toggle — but `wacManager.grantAccess()` already accepts `append: true`. The UI gap blocks the *self-service* path, not the capability. Verify end-to-end; the owner runs it, never the agent (Story 8.1 AC4's 403 must not be weakened).
- **Documentation correction required whatever the outcome:** `docs/team-onboarding.md` and `SKILL.md` promise Append-only "once 7.6/7.8 lands". Both were deferred to post-PoC on 2026-08-11, so that is now a promise against unscheduled work — on the one page whose entire purpose is honesty.
