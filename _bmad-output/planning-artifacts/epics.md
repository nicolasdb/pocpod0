...
- **Flags kept, not dropped:** swipe left/right still commit validated/rejected directly; drag-up reopens a multi-select tray — anagnorisis, follow-up, priority — same as the pre-Rate design, since these need practice before locking anagnorisis into the schema as a 3rd validation outcome (an earlier version of this story tried that and was reverted on user feedback). Flags + the restored comment field write to two new fields, `triage_flags` and `note`, deliberately not the pipeline's own `tags` (different meaning, already populated on every gist).
...
- **Schema gap open:** the handoff spec assumes `[gist_session]` carries `ingested = false`/`true`; the live file has no such key. Valisette defaults to "not ingested" when absent (safe today, incidental not contractual) — needs a decision with whoever owns the cleanup pass before the batch-closed path can be trusted.
