...
**CSS Stale-Session `accountId mismatch` Self-Repair (2026-09-01)**
Silent OIDC re-auth (`prompt=none`) is a **full-page redirect** in `@inrupt/solid-client-authn-browser`, not an iframe or token refresh. When the browser's `_session*` cookie no longer matches the client's stored Grant, `oidc-provider`'s `load_grant.js` throws `accountId mismatch` (or `clientId mismatch`, same file, same cure) and CSS surfaces a bare H500 blaming the server — stranding the user on `pod.nicolasdb.eu` before the app's own JS ever runs. No amount of client-side (backoffice/valisette) error handling can catch this; the fix has to live in the identity-page chrome CSS itself serves.

Fix: a script appended to `infra/css/main.html.ejs` (the bind-mount already used for Story 7.2 restyling) detects the mismatch text in the error body, flushes the session via `/.oidc/session/end` → `/.oidc/session/end/confirm` (same-origin, self-submitting logout form, live-verified to expire all four `_session*` cookies), then replays the original auth URL — turning the 500 into either an invisible repair or, if that fails once (30s one-shot guard against looping), a plain-language manual reset button. Account-cookie logout (`controls.account.logout`, costs a password re-entry) is an escalation path only, not the default. Full trace in memory `css_stale_session_accountid_mismatch.md`; deployed `96adb19`.

**General pattern this establishes:** any future CSS-identity-page failure mode that strands the user *before* our own app JS loads must be fixed in `infra/css/main.html.ejs`/CSS's own templates, not in backoffice/valisette — those never get a chance to run.
