...
Status: done — Tasks 1-5 implemented, deployed to VPS, code-reviewed (3-layer, all patches applied). Task 6.1-6.3 verified live; 6.4 deliberately deferred by Nicolas as non-blocking (see Dev Agent Record).
...
- [x] **Task 6 — Live verification (AC: 3, 5, 15)** — deployed to VPS 2026-08-19; 6.1-6.3 verified live by Nicolas; 6.4 deliberately deferred as non-blocking (see 6.4 below and Dev Agent Record).
...
| 2026-08-20 | **Closed to done.** Code review complete (see Review Findings): 1 decision resolved, 7 patches applied and verified against offline test scripts, 7 items deferred. Task 6.4 remains `[ ]` — Nicolas deliberately deferred it as non-blocking (runs when `agent-smithwhite` is actually retired), not a gap in this closure. |

### Review Findings

Code review of commit range `8234e98..HEAD` (3-layer: Blind Hunter, Edge Case Hunter, Acceptance Auditor). Two items the reviewers independently raised — the unlink-not-blocked-while-active gap and the generic mint-refusal error — were already logged in `deferred-work.md` (2026-08-20) from Nicolas's own live testing; not re-listed here.

- [x] [Review][Patch] Orphaned profile doc on double cleanup failure is silent — `createAgentIdentity`'s catch blocks only `console.error` when both the main step and `_cleanupOrphanDoc` fail; surface via toast, no persistence (decided with Nicolas, 2026-08-20) [backoffice/pod-api.js:628-664]
- [x] [Review][Patch] `unlinkWarning` copy omits AC8's third disclosure — neither branch mentions that WAC grants already written on other pods survive unlinking [backoffice/index.html:1976]
- [x] [Review][Patch] `createAgentIdentity` (RealBackend + DemoBackend) has no existing-identity check before writing — a second create at the same slug/webId silently overwrites the prior profile doc [backoffice/pod-api.js:799-808, 972-1000]
- [x] [Review][Patch] `listWebIdLinks` has no guard for a malformed/null JSON body from `controls.account.webId` — unhandled TypeError instead of a clean error [backoffice/pod-api.js:919-927]
- [x] [Review][Patch] AC3's public-readable verification uses an unanchored `/oidcIssuer/` substring test, not a parse — it also matches `oidcIssuerRegistrationToken` and only gives the right answer because of step ordering (token triple isn't written yet at verify-public time) [backoffice/pod-api.js:647]
- [x] [Review][Patch] `agentName` has no maximum length before being composed into the WebID/profile IRI [backoffice/pod-api.js:799-800]
- [x] [Review][Patch] Double-clicking Retry on an incomplete create can race two concurrent `retryCreateAgent` calls — no in-flight guard [backoffice/index.html:1570-1584]

- [x] [Review][Defer] `mintIdentityChoices` could resolve empty while a valid session exists (loadAgentIdentities still pending/failed), blocking minting entirely — needs live check, ties to Task 6's open live-verification work [backoffice/index.html:1526-1543] — deferred, needs Task 6 live verification first
- [x] [Review][Defer] No automated e2e/integration test added despite three live bugs caught on this exact code path this story (webIdLinks key/value, cache-buster, ownership-challenge premise) — deferred, pre-existing test-infra gap
- [x] [Review][Defer] AC7 lists "host pod" as an attribute to show; it's only visible as a substring of the raw WebID URL, not a distinct labeled field — deferred, minor
- [x] [Review][Defer] AC7's "linked-or-not" dimension is never exercised as `false` in the UI — every row the code can produce is implicitly linked, by design of AC6's cleanup-on-failure — deferred, may be intentional
- [x] [Review][Defer] Mint picker lets a user select any linked WebID including a pod-root, the exact Alex-incident shape, relying on blast-radius copy rather than a structural block — deferred, pre-existing design tradeoff (nudge not gate)
- [x] [Review][Defer] "Safe default" mint choice picks the first non-pod-root identity in whatever order the account API returns links — undocumented, implicitly order-dependent — deferred, minor
- [x] [Review][Defer] `podShortOf()` silently returns a wrong/empty pod name for WebID shapes that don't match the assumed `/podname/...` layout — deferred, minor
