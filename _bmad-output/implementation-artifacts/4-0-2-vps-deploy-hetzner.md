...
- [x] **Task 6: Discord integration validation** (AC5)
  - [x] 6.1: Root cause: reasoning models (nemotron) stream `content:""` + `reasoning:"..."` → OpenClaw counts 0 payloads. Fix: `models.providers` block in `openclaw.json` with `reasoning:true` flags routes through `openrouter-thinking` stream family. `thinkingDefault:off` globally. Troll → `openrouter/openai/gpt-5.4-nano`. All agents respond.
  - [x] 6.2: Isabelle heartbeat fired in #general — HEARTBEAT_OK, sparql-query ran, consent state reported.
  - [x] 6.3: Troll heartbeat fired in #security_logs — HEARTBEAT.md read, probe ran.
  - [x] 6.4: Smoke test confirmed 2026-04-16 ~14:00–14:17 UTC.

- [x] **Task 7: Operational runbook** (AC7)
  - [x] 7.1: `infra/vps/README.md` created — ssh, stack ops, health checks, WebUI pairing, rollback, env catalog, disk management.
  - [x] 7.2: Volume backup documented in README.md.
  - [x] 7.3: Memory entries updated: `infra_vps_deploy.md`, `infra_vps_structure.md`, `story_4_0_2_vps_patterns.md`.
  - [x] 7.4: deferred-work.md updated with pipeline tab idea.

- [x] **Task 8: Rollback plan documentation**
  - [x] 8.1: Factory reset + soft restart documented in `infra/vps/README.md`.
  - [x] 8.2: Fallback to local dev documented in README.md.
...
- **Agent turns returning 0 payloads — root cause (2026-04-16):** Reasoning models (nemotron, deepseek-r1) stream `"content":""` + `"reasoning":"..."` in SSE chunks. OpenClaw counts only non-empty `content` chunks as payloads → `payloads=0`. Not a missing API key. Fix: add `models.providers` block to `openclaw.json` with `reasoning:true` on reasoning models so OpenClaw routes them through the `openrouter-thinking` stream family. Also set `thinkingDefault:off` globally to suppress reasoning bleed in responses. Troll switched to `openrouter/openai/gpt-5.4-nano` (reasoning-capable, cheap).
- **models.providers block required for config-driven Docker deployments (2026-04-16):** OpenClaw's `openclaw onboard` writes model capability metadata (reasoning flags, context windows) to per-agent `models.json` in the volume. Seed-on-boot bypasses onboarding → no metadata → reasoning models silently fail. Declarative fix: `models.providers` block in `openclaw.json` with `mode: "merge"` is the equivalent for containerised deployments.
- **openclaw.json seed-on-first-boot (2026-04-16):** Entrypoint changed from always-overwrite to seed-on-first-boot (same pattern as workspaces). Bind-mount changed from `:ro` to `:rw`. OpenClaw can now write back config changes that persist across restarts. To apply repo changes to a live VPS: `ssh hetzner "cp /home/nicolas/pocpod0/agents/openclaw.json /var/lib/docker/volumes/pocpod0_openclaw-data-default/_data/openclaw.json && docker restart openclaw-gateway"`.
- **VOLUME_FLAGS=,Z SELinux bind-mount (2026-04-16):** Removing `:ro` without a replacement flag causes `,Z` (from VOLUME_FLAGS) to append to the destination path. Pattern: always use `:rw${VOLUME_FLAGS:-}` or `:ro${VOLUME_FLAGS:-}` to keep the comma-separator working.
