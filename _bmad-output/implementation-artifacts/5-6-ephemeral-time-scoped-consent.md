# Story 5.6: [done] Ephemeral Time-Scoped Consent — Double-Aveugle Pattern
...
- [x] Add `create_ephemeral_consent_grant(pod_name: str, service_label: str, purpose: str, scope: str, excluded: str, consequence_of_refusal: str, expires_at: str) -> EphemeralGrantResult` to `pipeline/src/pocpod0_pipeline/consent_grant.py`
- [x] Token generation: `hashlib.sha256(f"{pod_uri}{grant_timestamp}".encode()).hexdigest()[:16]`
...
- [x] Render the ephemeral Turtle template (Task 2) with all fields including `poc:token`
- [x] Store the token→pod_uri mapping in the Oxigraph token index (Task 3) — one triple: `<urn:token:{token_id}> poc:tokenFor <{pod_uri}>`
- [x] Emit `consent.grant` JSONL event with `token_id` field appended:
```json
...
```
- [x] Return `EphemeralGrantResult(grant_id, grant_uri, pod_name, token_id, expires_at, status, error_message="")`

### Task 2: Ephemeral consent grant Turtle template (AC1)
- [x] Create `infra/css/pods/ephemeral-consent-grant.ttl.j2` (Jinja2 template — separate from `consent-grant.ttl.j2` to avoid breaking Story 5.5 baseline)
- [x] Template variables: `grant_uri`, `service_label`, `token_id`, `purpose`, `scope`, `excluded`, `consequence_of_refusal`, `granted_at`, `expires_at`
- [x] Include `poc:token` predicate alongside `poc:requestedBy` (the token IS the identity of the requestor — the service has no WebID)
- [x] Rendered template:
```turtle
...
```
- [x] Declare `poc:EphemeralConsentGrant` (subclass of `poc:ConsentGrant`), `poc:token`, and `poc:serviceLabel` in `data/schemas/pocpod0-vocab.ttl`
- [x] Validate rendered Turtle with `rdflib` before writing to pod

### Task 3: Token lookup index in Oxigraph (AC1, AC3)
- [x] Implement `register_token(token_id: str, pod_uri: str) -> None` in `consent_grant.py`:
...
- [x] Implement `lookup_token(token_id: str) -> dict | None` in `consent_grant.py`:
...
- [x] Implement `deregister_token(token_id: str) -> None` in `consent_grant.py`:
...
- [x] Document that the food service NEVER queries `<urn:token-index>` — it only presents the token value when requesting data. The token lookup is a school-admin-only operation.

### Task 4: Expiry check script — pipeline step (AC2)
- [x] Create `pipeline/src/pocpod0_pipeline/expire_tokens.py`
- [x] Implement `check_expired_tokens() -> list[ExpiryResult]`:
...
- [x] Implement `revoke_expired_token(token_id: str, grant_id: str, pod_name: str) -> ExpiryResult`:
...
- [x] Implement CLI entrypoint: `python pipeline/src/pocpod0_pipeline/expire_tokens.py` — runs the expiry check once and exits
- [x] This script is intended to be called periodically by the pipeline or manually during demo — it is NOT a long-running daemon
- [x] Log each expiry with structured JSON to stdout

### Task 5: JSONL expiry event (AC2)
- [x] `expire_tokens.py` emits `consent.expired` event to `data/consent-events.jsonl` on each expired token:
```json
...
```
- [x] Reuse `emit_consent_event()` from `consent_grant.py` (extend its signature if needed to support `token_id` and `expired_at` fields)
- [x] Emit events are append-only to `data/consent-events.jsonl` — never overwrite
- [x] The mission control TUI (Story 3.7.1) should display `consent.expired` events in the consent events stream

### Task 6: Access log receipt written to Ayoub's pod (AC2)
- [x] On token expiry, write a Turtle receipt to `/ayoub/access-log/camp-food-[date].ttl` in the pod:
```turtle
@prefix poc: <http://localhost:3000/vocab/pocpod0#> .
@prefix xsd: <http://www.w3.org/2001/XMLSchema#> .
...
```
- [x] Create `infra/css/pods/access-receipt.ttl.j2` Jinja2 template for the receipt
- [x] The receipt is written to the pod via HTTP PUT (CSS authenticated as pod owner)
- [x] The receipt is append-style: if a receipt already exists for the same `camp-food-[date]`, use a unique suffix (e.g., `camp-food-[date]-2.ttl`)
- [x] Document that in the PoC, receipts are written by the pipeline (server-side); in the pilot, receipts would be generated by the CSS server automatically on access

### Task 7: Community pod setup for food service scenario (AC3)
- [x] Ensure the `school-community` pod exists in the CSS provisioning scripts (verify in `infra/css/pods/` and `pipeline/src/pocpod0_pipeline/provision_pods.py`)
- [x] Seed the community pod with aggregate dietary restriction data representing the camp enrollment:
...
- [x] Create or verify a SPARQL template `agents/skills/sparql-query/templates/camp-aggregate.rq` that queries the community pod named graph for dietary restriction counts
- [x] Verify the template returns aggregate counts only — no individual identification possible by query construction (structural anonymization per BP-2)
- [x] ACL on community pod: Isabelle's WebID has read access; food service token does NOT have access to the community pod (food service only receives its per-token data, not aggregates)

### Task 8: Double-aveugle demonstration test (AC1, AC2, AC3)
- [x] Create `tests/test_ephemeral_consent.py`
- [x] Test `create_ephemeral_consent_grant`: verify token is a 16-char hex string; verify Turtle contains `poc:token` and `poc:expiresAt`; verify token registered in Oxigraph index
- [x] Test token generation determinism: same `pod_uri` + `grant_timestamp` always produces same token
- [x] Test `lookup_token`: mock Oxigraph SPARQL response; verify correct `pod_uri` returned
- [x] Test `deregister_token`: verify SPARQL DELETE issued to `<urn:token-index>` with correct token URI
- [x] Test `check_expired_tokens`: mock Oxigraph response with 2 expired tokens + 1 active token; verify only the 2 expired tokens are revoked
- [x] Test `revoke_expired_token`: verify Turtle `poc:revokedAt` populated; ACL revoked; token deregistered; JSONL event emitted; access log receipt written
- [x] Test double-aveugle property: verify that the food service token value does NOT appear as a WebID in Ayoub's pod — the token is an opaque string, not a dereferenceable identity
- [x] Test aggregate query (community pod): verify camp-aggregate.rq returns only counts; verify no individual WebID appears in query result
- [x] Test access log receipt: verify receipt Turtle is written to correct path with correct `poc:accessGrantedAt` and `poc:accessExpiredAt` values
- [x] Minimum 12 unit tests; all must pass (25 unit tests implemented)

### Task 9: Integration tests (AC1, AC2, AC3)
- [x] Create `tests/test_ephemeral_consent_integration.py` (or extend `test_ephemeral_consent.py` with integration markers)
- [x] Integration test: full lifecycle — create ephemeral grant → verify token in Oxigraph index → simulate expiry (mock `datetime.now()` past `expires_at`) → run expiry check → verify tombstone in Turtle → verify JSONL event emitted → verify receipt written → verify ACL revoked
- [x] Integration test: double-aveugle — verify food service cannot query `<urn:token-index>` (SPARQL ACL enforcement on the named graph)
- [x] Integration test: aggregate query still works after Ayoub's individual token expires (community pod data is independent of Ayoub's pod lifecycle)
- [x] Mark integration tests with `@pytest.mark.integration` to allow selective exclusion from unit-test-only runs
...
- **Test Environment Mismatch (Story 5.6 Validation)**: 
  - **Issue**: `TestRevokeExpiredToken.test_deregisters_token` failed. Expected `deregister_token` to be called with `http://localhost:7878`, but it was called with `http://oxigraph:7878`.
  - **Root Cause**: The `.env` file overrides the default `_OXIGRAPH_URL` to use the Docker-internal hostname `oxigraph`, while the test had the `localhost` value hardcoded in its assertion.
  - **Fix**: Updated `tests/test_ephemeral_consent.py` to import and use the `_OXIGRAPH_URL` constant from `pocpod0_pipeline.expire_tokens`, ensuring the test assertion automatically adapts to the environment configuration.
- **Legacy Fix**: `fake_revoke` in `test_returns_only_expired` needed `expired_at` kwarg to match `revoke_expired_token` signature.

### Completion Notes List

- **Task 1+3**: Extended `consent_grant.py` with `EphemeralGrantResult` dataclass,
  `generate_token()` (SHA-256[:16], deterministic), `register_token()`, `lookup_token()`,
  `deregister_token()` (all using httpx for Oxigraph, consistent with delete_cascade.py),
  `_render_ephemeral_turtle()`, `create_ephemeral_consent_grant()` with full
  atomicity/rollback (token registered before CSS write, deregistered on CSS failure).

- **Task 2**: Created `ephemeral-consent-grant.ttl.j2` (separate from consent-grant.ttl.j2
  to preserve Story 5.5 baseline). Added `poc:EphemeralConsentGrant`, `poc:token`,
  `poc:serviceLabel`, `poc:AccessReceipt`, `poc:DietaryAggregate` and associated
  properties to `pocpod0-vocab.ttl`.

- **Task 4+5+6**: Created `expire_tokens.py` — `check_expired_tokens()` queries
  `<urn:token-index>` for expired tokens and calls `revoke_expired_token()` for each.
  `revoke_expired_token()` applies tombstone, deregisters token, writes access receipt
  (non-fatal on failure), emits `consent.expired` JSONL event. Idempotent: returns
  `already_revoked` if `poc:revokedAt` already populated.

- **Task 7**: Verified `school-community` pod exists in `pod-config.yaml`. Created
  `data/seeds/camp-dietary-aggregate.ttl` (47 enrollments, 4 restriction categories,
  no individual WebIDs). Created `camp-aggregate.rq` SPARQL template with SUM aggregates
  on community pod named graph — structural anonymization via aggregate-only SELECT.

- **Tasks 8+9**: 25 unit tests + 6 integration tests, all passing. Total test suite:
  167 tests (no regressions introduced). Integration tests cover full lifecycle,
  tombstone Turtle verification, idempotency, double-aveugle property, aggregate query
  structural anonymization, and community pod seed data cleanliness.

### File List

pipeline/src/pocpod0_pipeline/consent_grant.py
pipeline/src/pocpod0_pipeline/expire_tokens.py
infra/css/pods/ephemeral-consent-grant.ttl.j2
infra/css/pods/access-receipt.ttl.j2
data/schemas/pocpod0-vocab.ttl
data/seeds/camp-dietary-aggregate.ttl
agents/skills/sparql-query/templates/camp-aggregate.rq
tests/test_ephemeral_consent.py
tests/test_ephemeral_consent_integration.py

### Change Log

- 2026-03-30: Story 5.6 implemented — BP-5 ephemeral time-scoped consent (double-aveugle pattern).
  New: expire_tokens.py, ephemeral-consent-grant.ttl.j2, access-receipt.ttl.j2,
  camp-dietary-aggregate.ttl, camp-aggregate.rq. Modified: consent_grant.py (ephemeral
  functions), pocpod0-vocab.ttl (new classes/properties). 31 tests added.
