...
- 2026-07-31 (Task 1 complete): Deploy plumbing done and verified.
  ...
  - Also wired `PORT`/`HOST`/`ALLOWED_HOSTS` as compose `environment:` (env `MCP_ALLOWED_HOSTS`, default `mcp.nicolasdb.eu`, added to root `.env` — local-only value has no effect since local dev connects via `127.0.0.1` directly, bypassing the Host check).
...
- `.env` — added `MCP_ALLOWED_HOSTS=mcp.nicolasdb.eu` (local-only file, not committed).
