...
| Backoffice grant granularity | **No Append-only option in the backoffice UI.** Only RO / RW / only-me / public-read exist there — but Append-only is now proven reachable and live via `wacManager.grantAccess({append:true})`, run by the operator as OWNER. Self-service through the backoffice is still blocked (7.6/7.11b, deferred post-PoC); the capability itself is not. | 8.6 session finding 2026-08-02, verified live by Story 8.9 spike 2026-08-11 |
...
7. **Honest limits are stated plainly, not buried.** At minimum: (a) destructive-action confirmations are MCP *hints* — record what claude.ai actually did (8.5: two layers fired), and say the guarantee rests on the client honouring them; (b) read receipts are a **voluntary convention** — CSS surfaces no per-resource read log, so a reader that declines to write receipts leaves no trace; (c) there is **no pod versioning** — append is the safety mechanism, overwrite can lose data permanently; (d) the slug **is** a credential — anyone holding that URL is you, to the connector; (e) **your `access-log/` grant is Append-only, verified live** — your agent (and anyone else's writing into your log) can add entries but never read, edit, or delete them; you, the owner, retain full Control and can still edit your own log, so the log is evidence against them, not against you; minting this grant is not yet self-service through the backoffice (7.6/7.11b, deferred post-PoC), so today it's set by the operator on request.
...
- **The backoffice has no Append-only grant option.** Only RO/RW/only-me/public-read exist there. Append-only is real and live — proven end-to-end by Story 8.9 (`wacManager.grantAccess({append:true})`, verified against the deployed pod) — but minting it is a scripted operator step, not yet a backoffice click (7.6/7.11b, deferred post-PoC).
