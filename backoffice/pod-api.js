...
export const Solid = {
  ...
  async _session() {
    if (!_backofficeSession) {
      ...
      _backofficeSession = { session: new libs.authn.Session({}, BACKOFFICE_SESSION_ID), libs };
    }
    return _backofficeSession;
  },
  async init() {
  ...
  async connect(oidcIssuer, clientName) {
    const { session } = await this._session();
    await session.login({
  ...
  async disconnect() {
    try { const { session } = await this._session(); await session.logout(); } catch (e) {}
  },
  async realClient() {
    const { session, libs } = await this._session();
